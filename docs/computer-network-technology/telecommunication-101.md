...
- 数据编码
  - 将数据用物理量表示的方法。例如字符“A”的编码为*01000001*，其数据编码可为：![image-20221028170426877](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028170426877.png)。
...
> 例：NRZ编码一个码元表示一个比特的信息，因此 $C=B$；四进制码有4种码元状态（00、01、10、11），每个码元带2比特数据，所以 $C=2B$。![image-20221028170335602](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028170335602.png)
...
- 双绞线：
  ![image-20221028173431762](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028173431762.png)
  - 特点：
    - 螺旋绞合的双导线
    - 每根4对、25对、1800对
    - 典型连接距离100m（LAN）
...
- 屏蔽双绞线（STP）：以铝箔屏蔽以减少干扰和串音，应用较少。
  ![image-20221028173725207](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028173725207.png)
- 非屏蔽双绞线（UTP）：双绞线外无任何屏蔽层，应用广泛。
  ![image-20221028173845699](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028173845699.png)
...
![image-20221028174745339](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028174745339.png)
...
![image-20221028175029079](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028175029079.png)

光纤可分为**单模光纤和多模光纤**。

- 多模光纤（MMF）：由二极管发光，**低速短距离**，2千米以内。多模光纤的芯线粗，传输速度低、距离短，整体的传输性能差，但其成本比较低，一般用于建筑物内或地理位置相邻的建筑物间的布线环境下，
  ![image-20221028175052189](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028175052189.png)
- 单模光纤（SMF）：由激光作光源，仅有一条光通路，**传输距离长**，2千米以上。单模光纤是当前计算机网络中研究和应用的重点，也是光纤通信与光波技术发展的必然趋势。
  ![image-20221028175101795](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028175101795.png)

为了保护光纤，我们需要为其外包保护层，形成**光缆**。光缆可分为**单芯光缆和多芯光缆**，构造分别如下图。

![image-20221028175403056](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028175403056.png)
...
![image-20221028175928695](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028175928695.png)
...
- 单工、半双工和全双工；
  ...
  - 全双工传输：两设备之间有两个通道，他们可以同时在两个方向上传输数据。如计算机之间的数据通信等。
    ![image-20221028185745283](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028185745283.png)
- 基带传输和频带传输；
  ...
  - 将数字数据转换成模拟信号，利用模拟信道进行传输的方法称为频带传输；频带传输借助于模拟的正弦载波信号，用数字数据（0或1）调制载波，将数字数据寄生在载波的某个参数上（振幅、频率和相位），借助于模拟信道进行传输。它需要利用调制解调技术对数据进行转换。频带传输可以利用**频分多路复用**（FDM）实现多路复用，提高传输信道的利用率。
    ![image-20221028194224454](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028194224454.png)
  ...
  - 通过借助频带传输，**将信道分成多个子信道**，分别传送音频、视频和数字信号，称为**宽带传输**。宽带是指比音频带宽更宽的频带，它包括大部分电磁波频谱。使用这种宽频带传输的系统，称为宽带传输系统。即**把信号调制成频带为几十MHz到几百MHz的模拟信号**后再传送，接收方需要解调。宽带传输中的所有信道都可以同时发送信号。如CATV、ISDN等。
    ![image-20221028194351935](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028194351935.png)
    ![image-20221028194442045](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028194442045.png)
- 同步传输和异步传输等。
...
- 位——位同步
  ...
  - **外同步**——发送端发送数据之前发送同步脉冲信号，接收方用接收到的同步信号来锁定自己的**时钟脉冲频率**。
    ![image-20221028190054318](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028190054318.png)
...
- 同步传输方式中以**称为帧的数据块为单位**进行传送。同步传输使用特殊的标志位进行帧同步，界定一个帧的始末。由于一个信息帧可以包含的位数很多，**同步传输还必须进行严格的位同步**，要求通信双方使用同一时钟信号进行发送与接收。
  ![image-20221028190325772](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028190325772.png)
- 异步传输以字符为单位进行数据传输。每个字符前后通过加起始位和停止位来实现字符同步，通信的双方各自使用独立的基准定位时钟，但**要约定同样的传输速率**，以实现位同步。
  ...
  4. 1～2位停止位，以逻辑"1"表示，用作字符间的间隔。
     ![image-20221028190627428](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028190627428.png)
...
**串行传输和并行传输**的效果示意图如下：
![image-20221028191144066](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028191144066.png)

| 串行转为并行                                                   | 并行转为串行                                                   |
| -------------------------------------------------------------- | -------------------------------------------------------------- |
| ![image-20221028191626238](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028191626238.png) | ![image-20221028191616911](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028191616911.png) |

## 数据编码技术

数据编码类型总览：
![image-20221028194541638](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028194541638.png)

模拟数据编码即**频带传输的调制解调技术**等：
![image-20221028194605810](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028194605810.png)
...
![image-20221028200641977](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028200641977.png)

## 多路复用技术

把两个以上的单独的信号合并起来，同时在一个通信线路上进行传输。这被称为**多路复用**。
![image-20221028201250225](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028201250225.png)
...
![image-20221028201512872](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028201512872.png)
...
![image-20221028210724325](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028210724325.png)
...
![image-20221028210950993](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028210950993.png)
...
![image-20221028211517277](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028211517277.png)
...
- 报文交换
  ...
  - 适合于长报文、**无实时要求的通信**、不适合会话式通信。
    ...
    ![image-20221028211656613](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028211656613.png)
...
- 分组交换
  ...
  - 分组交换又可分为**数据报分组交换和虚电路分组交换**两种。
    ...
    ![image-20221028211805718](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028211805718.png)

数据报方式是通信子网将进入子网的分组当作“小报文”处理，**每个分组独立寻径，分组通常称**数据报\*\*\*\*。

![image-20221028211838937](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028211838937.png)
...
![image-20221028212015016](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028212015016.png)

![image-20221028212118741](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028212118741.png)
...
![image-20221028212352982](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028212352982.png)
...
![image-20221028212450328](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028212450328.png)
...
![image-20221028212659287](https://raw.githubusercontent.com/xrl-workshop/cdn/main/image-20221028212659287.png)
